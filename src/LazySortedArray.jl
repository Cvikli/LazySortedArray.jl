...
#####
# 1. 
# Not enough good. Create the LazySortedArray with two separate array: "sorted_data" and "lazy_data", to have a "limit" of data they keep track of. Also please exclude the newly inserted data if the data is not above certain threshold. Also please don't forget to merge. Also please add comments on how could you optimize it more.
# 
...
@kwdef  mutable struct LazySortedArray{T} <: LazySortedContainer
	sorted_arr::Vector{T}
	sptr::Int
	threshold::T
	lazy_arr::Vector{T}
...
LazySortedArray(data::Vector{T}) where T    = LazySortedArray{T}(data, length(data), minimum(data, init=nextfloat(typemin(T))), Vector{T}(undef,max(10,cld(lenght(data),10))), 0)
LazySortedArray(T::Type)                    = LazySortedArray{T}(zeros(T,1_000_000), 0, nextfloat(typemin(T)), Vector{T}(undef,10_000), 0)
LazySortedArray(T::Type, mainsize)          = LazySortedArray{T}(zeros(T,mainsize),  0, nextfloat(typemin(T)), Vector{T}(undef,10_000), 0)
LazySortedArray(T::Type, mainsize, subsize) = LazySortedArray{T}(zeros(T,mainsize),  0, nextfloat(typemin(T)), Vector{T}(undef,subsize), 0)


laziness_factor(lsa) = (length(lsa.sorted_arr)/length(lsa.lazy_arr))
is_sorted(lsa::LazySortedContainer)                         = lsa.lptr==0
whole_length(lsa::LazySortedArray{T}) where T               = length(lsa.sorted_arr)
Base.length(lsa::LazySortedArray{T})  where T               = (ensure_sorted(lsa); return lsa.sptr)
Base.getindex(lsa::LazySortedArray{T}, index::Int) where T  = (ensure_sorted(lsa); return lsa.sorted_arr[index])
...
Base.insert!(lsa::LazySortedArray{T}, st::T; threshold=lsa.threshold) where T = if st >= threshold 
...
Base.merge!(lsa::LazySortedArray{T}, lsort=lsa.lazy_arr) where T = begin
	N = whole_length(lsa)
...
Base.show(io::IO, lsa::LazySortedArray{T}) where T = (evaluate(lsa); println(io,"min=$(lsa.threshold) arr=$(lsa.sorted_arr[1:min(50,length(lsa))])$(length(lsa)>50 ? "..." : "")"))





end # module LazySortedArray
